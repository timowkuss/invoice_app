...
document.addEventListener('submit',async event=>{
  ...
  try {
    if(form.id==='auth-form'){const signup=form.dataset.signup==='true';const result=await api(`/auth/${signup?'signup':'login'}`,{method:'POST',body:JSON.stringify(Object.fromEntries(data))});user=result.user;selectedStore='';sessionStorage.removeItem('store');shell();await openLanding();}
...
(async()=>{localStorage.removeItem('token');try{user=await api('/auth/me');shell();await openLanding();}catch{authView();}})();
